...
def eza-wrapper [
    ...
    if ($dir == "") {
       ...
       eza ($flags) --color=always --color-scale=all --color-scale-mode=fixed --icons=auto --level=($level) --sort=type --git --git-ignore --group-directories-first --hyperlink --no-permissions --no-user --total-size $dir
    }
}
